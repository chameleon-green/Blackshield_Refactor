...
if(Click) {
	...
	if(Tab = "items") {
		if(ButtonRegionCenter(Mouse_X,Mouse_Y,302,272,140,170,scale) && (SubTab != "weapons")) {SubTab = "weapons"; bg_subimage = 0; counter_weapons = 0; global.Selected = undefined};
		if(ButtonRegionCenter(Mouse_X,Mouse_Y,264,234,140,170,scale) && (SubTab != "armor")) {SubTab = "armor"; bg_subimage = 1 counter_armor = 0; global.Selected = undefined};
		if(ButtonRegionCenter(Mouse_X,Mouse_Y,226,196,140,170,scale) && (SubTab != "aid")) {SubTab = "aid"; bg_subimage = 2 counter_aid = 0; global.Selected = undefined};
		if(ButtonRegionCenter(Mouse_X,Mouse_Y,188,158,140,170,scale) && (SubTab != "ammo")) {SubTab = "ammo"; bg_subimage = 3 counter_ammo = 0; global.Selected = undefined};
		...
		if(ButtonRegionCenter(Mouse_X,Mouse_Y,112,82,140,170,scale) && (SubTab != "crafting")) {SubTab = "crafting"; bg_subimage = 5 counter_crafting = 0; global.Selected = undefined};
	};
...
if(Tab = "items") {
	...
	if(SubTab = "weapons") {
		while(counter_weapons < (InventorySize-1)) {
			var Slot = ds_grid_get(grd_inv_wepn,0,counter_weapons)
			if(Slot != 0) {				
				var ID = ds_grid_get(grd_inv_wepn,8,counter_weapons);
