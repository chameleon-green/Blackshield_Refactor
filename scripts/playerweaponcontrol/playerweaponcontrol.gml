...
function FireGun (){
	...
	audio_sound_pitch(FireSound, random_range(0.7,1));	
};	
#endregion
...
function PlayerWeaponControl(){	
	...
	var Semi_Fire = mouse_check_button_pressed(mb_left) and (selector = "Semi" or selector = "Supercharge");
	...
	if(mouse_check_button_pressed(mb_left) and magazine_active = 0) {audio_play_sound_at(wpn_active.sound_group.empty,x,y,0,100,100,1,0,1)};
	...
	if(Reload_Key and _CanReload) {
		...
		skeleton_animation_set_ext(wpn_active.animation_group.reload,4);
	};
